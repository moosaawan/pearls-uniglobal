...
import { ArrowLeft, ArrowRight, CheckCircle, ClipboardCheck, User, GraduationCap, Settings } from 'lucide-react'
...
export default function FreeAssessmentPage() {
  ...
  const { register, handleSubmit, control, formState: { errors }, watch, trigger } = useForm<AssessmentFormData>({
  ...
  const nextStep = async () => {
    const fieldsToValidate: Record<number, (keyof AssessmentFormData)[]> = {
  ...
  const onSubmit = async () => {
    ...
    setSubmitted(true)
    toast.success('Assessment submitted! We\'ll contact you within 24 hours.')
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center section-padding">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center max-w-md">
          ...
          <h1 className="text-3xl font-bold text-foreground mb-3">Assessment Submitted!</h1>
          <p className="text-muted-foreground mb-8 font-sans">Thank you for your interest. Our expert counselors will review your profile and contact you within 24 hours.</p>
      ...
      <section className="bg-gradient-hero text-white py-16 md:py-20 px-4 relative overflow-hidden">
        ...
        <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="container-wide relative z-10 text-center">
          ...
          <motion.p variants={fadeUp} className="text-white/70 max-w-xl mx-auto font-sans">
            Fill out this form and our experts will evaluate your profile for UK university admission.
          </motion.p>
        </motion.div>
      </section>
            ...
            <div className="flex items-center justify-between mb-4">
              {steps.map((step) => (
                <div key={step.id} className="flex items-center gap-2">
                  ...
                  <span className="hidden sm:block text-sm font-medium text-foreground font-sans">{step.title}</span>
                </div>
              ))}
            </div>
            ...
            <CardContent className="p-8">
              <form onSubmit={handleSubmit(onSubmit)}>
                <AnimatePresence mode="wait">
                  ...
                  {currentStep === 4 && (
                    <motion.div key="step4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                      ...
                      <p className="text-muted-foreground text-sm mb-6 font-sans">Please review before submitting</p>
                ...
                <div className="flex justify-between mt-8 pt-6 border-t border-border">
                  <Button type="button" variant="outline" onClick={prevStep} disabled={currentStep === 1} className="rounded-xl font-sans">
                  ...
                  {currentStep < 4 ? (
                    ...
                    <Button type="submit" className="bg-gold hover:bg-gold-dark text-navy rounded-xl font-semibold font-sans">
